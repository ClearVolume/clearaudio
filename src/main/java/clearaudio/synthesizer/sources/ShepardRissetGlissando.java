 ...
 *
 */
public class ShepardRissetGlissando extends SourceBase
                                    implements Source
{
                                ...
                                float pFrequencySpacing)
  {
    ...
    for (float lFrequency =
                          mBottomFrequency; lFrequency < mTopFrequency; lFrequency +=
                                                                                   mFrequencySpacing)
    {
  ...
  private float getAmplitudeEnvelopp(float pFrequency)
  {
    if (pFrequency < mBottomFrequency || pFrequency > mTopFrequency)
      return 0;
    float lNormalizedFrequency = (pFrequency - mBottomFrequency)
                                 / (mTopFrequency - mBottomFrequency);
    float lAmplitude = (float) (getAmplitude()
                                * pow(sin(PI * lNormalizedFrequency),
                                      2));
    return lAmplitude;
  }
  ...
  public int getPeriodInSamples()
  {
    ...
    for (Sinusoid lSinusoid : mSinusoidList)
    {
      ...
      lMaxPeriodInSamples =
                          max(lMaxPeriodInSamples, lPeriodInSamples);
    }
