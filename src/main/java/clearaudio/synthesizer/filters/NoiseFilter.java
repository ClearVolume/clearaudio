package clearaudio.synthesizer.filters;
...
public class NoiseFilter extends FilterBase
{
  ...
  public float next()
  {
    ...
    ThreadLocalRandom lThreadLocalRandom =
                                         ThreadLocalRandom.current();

    double lAdditiveNoise =
                          2 * (lThreadLocalRandom.nextFloat() - 0.5);
    double lMultiplicativeNoise =
                                0.2 * (lThreadLocalRandom.nextFloat()
                                       - 0.5);
    double lOutValue =
                     lInValue + getAmplitude()
                                * (lAdditiveNoise
                                   + lInValue * lMultiplicativeNoise);
