...
public class ReverbFilter extends FilterBase
{
  ...
  public ReverbFilter(float pAmplitude, float pReverbPeriodInSeconds)
  {
    ...
    final int lReverbPeriodInSamples =
                                     Math.round(getSamplingFrequency()
                                                * pReverbPeriodInSeconds);
  ...
  public void setDefaulKernel()
  {
    ...
    for (int r = 0; r < mReverbBuffer.length / 4; r++)
    {
      ...
      for (int j = 1; j < mReverbBuffer.length - 1; j++)
        mReverbBuffer[j] = 0.99f * (mReverbBuffer[j - 1]
                                    + 2 * mReverbBuffer[j]
                                    + mReverbBuffer[j + 1])
                           / 4;/**/
      mReverbBuffer[mReverbBuffer.length
                    - 1] =
                         0.5f
                           * (mReverbBuffer[mReverbBuffer.length - 2]
                              + mReverbBuffer[mReverbBuffer.length
                                              - 1]);
    }
